...
template: splash
hero:
  tagline: Govern every AI agent like a first-class identity — owned by a human, granted task-scoped access, elevated only on approval, and proven on a tamper-evident audit trail. One /authz decision for every request and every agent's outbound call.
  actions:
...
it — any OIDC/SCIM IdP integrates the same way. Logto is the reference IdP used in demos and
seeded datasets; see the [IdP Support Model](/docs/concepts/idp-support/). For the security
